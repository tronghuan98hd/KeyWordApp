...
class NDFrame(PandasObject, SelectionMixin, indexing.IndexingMixin):
    ...
    def __neg__(self):
        ...
        if is_bool_dtype(values):
            ...
            raise TypeError(
                f"Unary negative expects numeric dtype, not {values.dtype}")
    ...
    def __pos__(self):
        ...
        if is_bool_dtype(values) or is_period_arraylike(values):
            ...
            raise TypeError(
                f"Unary plus expects numeric dtype, not {values.dtype}")
        return self.__array_wrap__(arr)
    ...
    def _drop_labels_or_levels(self, keys, axis: int = 0):
        ...
        levels_to_drop = [
            k for k in keys if self._is_level_reference(k, axis=axis)]

        labels_to_drop = [
            k for k in keys if not self._is_level_reference(k, axis=axis)]
        ...
        if self._is_view and self._is_cached:
            ...
            if ref is not None and ref._is_mixed_type:
                self._check_setitem_copy(
                    stacklevel=4, t="referant", force=True)
            return True
        elif self._is_copy:
    ...
    **kwargs
        ...
        if is_dict_like(dtype):
            ...
            for col_name, col in self.items():
                if col_name in dtype:
                    results.append(
                        col.astype(dtype=dtype[col_name],
    ...
    def replace(
        ...
        if value is None:
            ...
            if isinstance(to_replace, (tuple, list)):
                if isinstance(self, ABCDataFrame):
                    return self.apply(
                        _single_replace, args=(
                            to_replace, method, inplace, limit)
                    )
            ...
            if is_dict_like(to_replace):
                ...
                # {'A': NA} -> 0
                    ...
                    keys = [(k, src)
                            for k, src in to_replace.items() if k in self]
                    keys_len = len(keys) - 1
    ...
    def _align_series(
        ...
        if fill_na:
            left = left.fillna(fill_value, method=method,
                               limit=limit, axis=fill_axis)
    ...
    def truncate(
        ...
        if isinstance(ax, MultiIndex):
            setattr(result, self._get_axis_name(
                axis), ax.truncate(before, after))
    ...
    def describe(
        ...
        d = pd.concat([x.reindex(names, copy=False)
                       for x in ldesc], axis=1, sort=False)
    ...
    def pct_change(
        ...
        **kwargs,
        ...
        rs = data.div(data.shift(periods=periods,
                                 freq=freq, axis=axis, **kwargs)) - 1
        if freq is not None:
        ...
        def na_accum_func(blk_values):
            ...
            if blk_values.dtype.kind in ["m", "M"]:
                ...
                if isinstance(blk_values, np.ndarray):
                    ...
                    # DatetimeArray
                    result = type(blk_values)._from_sequence(
